...
public class RegistroService {
    ...
    public RegistroService(CorreoService correoService,
        ...
        String baseServidor = constante.map(Constante::getValor).orElse("localhost");

        // Limpia cualquier esquema previo enviado desde la BD
        baseServidor = baseServidor.replace("http://", "").replace("https://", "");

        if (baseServidor.startsWith("localhost") && !baseServidor.contains(":")) {
            baseServidor = baseServidor + ":" + serverPort;
        }

        // Reintegra el protocolo http:// obligatorio para validez del enlace cliqueable
        this.servidor = "http://" + baseServidor;
    }
    ...
    public Model activar(Model model, String username, String clave) {
        ...
        if (usuario.isPresent()) {  //Si estaba...
